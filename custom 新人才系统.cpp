...
<<<<<<< HEAD
				//init_history_new_person(new_person);//覆盖，产生历史武将
=======
				bool result0 = false;
...
<<<<<<< HEAD
		void init_history_new_person(pk::person@ person)
		{
			int id = pk::rand(183);
			...
			for (int i = 0; i < 武将能力_末; ++i)
			{
				person.base_stat[i] = int(1.1f * 历史武将数据[id][i]);
				//pk::trace("i:" + i + ",stat:" + person.base_stat[i]);
			}
			person.update();
			pk::set_face(person, 历史武将数据[id][5]);
			person.sex = 历史武将数据[id][6];
=======
