...
// ## 2020/09/30 # 江东新风 # 港关伪报city指针错误修复，已着火对象也会被点火 ##
// ## 2020/09/23 # 氕氘氚 # 金钱65535上限修复 ##
// ## 2020/07/26 ##
/*
@제작자: HoneyBee
@설명: 도시 근처에 있는 적군 부대에게 위보 효과 (규율,명경,통찰,신산 특기를 소유한 부대는 면역)
...
namespace 据点计略_伪报
{
	...
	class Main
	{
		...
		string getDesc()
		{
			...
			case 0: return pk::encode(pk::format("伪报周围敌军.(消耗：金{} 行动力{})", GOLD_COST, ACTION_COST));
			default:
				return pk::encode("");
			}
			return pk::encode("");
		...
		int check_avaliable(pk::building@ building, pk::list<pk::person@> list)//之所以加入list是为了ai调用时不用重复计算，玩家菜单稍微多点操作问题不大
		{
			...
			else if (base_ex[building.get_id()].misinform_done) return 2;
		...
		bool handler()
		{
			...
			if (pk::choose({ pk::encode("  是  "), pk::encode("  否  ") }, pk::encode(pk::format("是否执行伪报?\n(资金 {} 使用)\n(对敌军部队造成伪报效果)", GOLD_COST)), taishu_) == 1)
				return false;
		...
		void onAIRunningOrder(pk::ai_context@ context, pk::building@ building, int cmd)
		{
			if (cmd == 据点AI_计略伪报)
			{
				...
				else if (调试模式) pk::trace("据点AI_计略伪报 不满足");
			}
		}
		...
		bool run_order_before(pk::list<pk::person@>& out person_sel, pk::building@ building0)
		{
			...
			for (int j = 0; j < int(arr.length); j++)
			{
				...
				if (dst_unit is null or dst_unit.status == 部队状态_伪报 or !pk::is_enemy(building0, dst_unit)) continue;
				...
				pk::int_bool result0 = STRATEGY_CHANCE::cal_strategy_chance(building0, person_sel, dst_unit.pos, 据点计略_伪报);//int_bool 成功率 是否被看破
		...
		bool run_order(pk::list<pk::person@> person_sel, pk::building@ building0)
		{
			...
			for (int j = 0; j < int(arr.length); j++)
			{
				...
				{
					count += 1;//影响部队技计数
					pk::set_status(dst_unit, null, 部队状态_伪报, 范围伪报回合, true);		//混乱
			...
			if (pk::is_in_screen(src_pos))
			{
				if (count > 1)
				{
					ch::say_message(7095, /*p0*/person_sel[0], /*p1*/null, /*u0*/null, /*p0_u*/building0);//直接调用原版伪报台词
				}
				else
				{
			...
			base_ex[building0.get_id()].misinform_done = true;
