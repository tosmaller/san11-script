// ## 2022/01/17 # messi # 制作减少范围内敌军士气的待劳指令 ##
...
namespace 据点计略_待劳
{
	...
	class Main
	{
		...
		string getDesc()
		{
			...
			case 0: return pk::encode(pk::format("执行待劳.(消耗：金{} 行动力{})", GOLD_COST, ACTION_COST));
			default:
				return pk::encode("");
			}
			return pk::encode("");
		...
		bool handler()
		{
			...
			if (pk::choose({ pk::encode("  是  "), pk::encode("  否  ") }, pk::encode(pk::format("是否执行待劳计谋的操作?\n(使用资金 {} )\n(降低据点周围敌方部队的士气)", GOLD_COST)), taishu_) == 1)
				return false;
		...
		void onAIRunningOrder(pk::ai_context@ context, pk::building@ building, int cmd)
		{
			if (cmd == 据点AI_计略待劳)
			{
				...
				else if (调试模式) pk::trace("据点AI_计略待劳 不满足");
			}
		}
		...
		bool run_order_before(pk::list<pk::person@>& out person_sel, pk::building@ building0)
		{
			...
			for (int j = 0; j < int(arr.length); j++)
			{
				...
				pk::int_bool result0 = STRATEGY_CHANCE::cal_strategy_chance(building0, person_sel, dst_unit.pos, 据点计略_待劳);//int_bool 成功率 是否被看破
		...
		bool run_order(pk::list<pk::person@> person_sel, pk::building@ building0)
		{
			...
			for (int j = 0; j < int(arr.length); j++)
			{
				...
				{
					count += 1;//影响部队技计数
					pk::add_energy(dst_unit, -待劳减气, true);

					if (pk::is_in_screen(dst_unit.get_pos()))
					{
						switch (pk::rand(2))
						{
						case 0: pk::say(pk::encode("此城固若金汤，将士们出现了懈怠!"), pk::get_person(무장_병사), dst_unit); break;
						case 1: pk::say(pk::encode("久攻不下，士气低沉.."), pk::get_person(무장_병사), dst_unit); break;
						}
					}
				}
				else//计策失败
